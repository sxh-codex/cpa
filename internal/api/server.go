...
type serverOptionConfig struct {
	...
	openAIUsageStore      openaiusage.Store
...
func WithOpenAIUsageStore(store openaiusage.Store) ServerOption {
	return func(cfg *serverOptionConfig) {
		cfg.openAIUsageStore = store
	}
}
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	s := &Server{
		...
		openAIUsageStore:    optionState.openAIUsageStore,
	...
	s.mgmt.SetOpenAIUsageStore(optionState.openAIUsageStore)
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.GET("/openai-usage/status", s.mgmt.GetOpenAIUsageStatus)
		mgmt.GET("/openai-usage/accounts", s.mgmt.GetOpenAIUsageAccounts)
		mgmt.GET("/openai-usage/accounts/:auth_index", s.mgmt.GetOpenAIUsageAccount)
		...
		mgmt.POST("/auth-files/remove-invalid", s.mgmt.RemoveInvalidAuthFiles)
		mgmt.POST("/auth-files/remove-401-invalid", s.mgmt.Remove401InvalidAuthFiles)
...
func (s *Server) SetOpenAIUsageStore(store openaiusage.Store) {
	if s == nil {
		return
	}
	s.openAIUsageStore = store
	if s.mgmt == nil {
		return
	}
	s.mgmt.SetOpenAIUsageStore(store)
}

...
func (e *CodexExecutor) ShouldPrepareRequestAuth(auth *cliproxyauth.Auth) bool {
	return helps.IsAgentIdentityAuth(auth) && strings.TrimSpace(helps.AuthMetadataString(auth, "task_id")) == ""
}
...
func (e *CodexExecutor) ShouldRecoverTask(auth *cliproxyauth.Auth, err error) bool {
	if !helps.IsAgentIdentityAuth(auth) || err == nil {
		return false
	}
	...
	return helps.IsAgentIdentityTaskInvalidResponse(status, []byte(err.Error()))
}

func (e *CodexExecutor) RecoverTask(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	if auth == nil || !helps.IsAgentIdentityAuth(auth) {
		return auth, nil
	}
	return e.registerAgentIdentityTask(ctx, auth)
}
...
func (e *CodexExecutor) registerAgentIdentityTask(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	taskID, err := helps.RegisterAgentIdentityTask(ctx, client, helps.AgentIdentityAuthAPIBaseURL, updated, time.Now())
	if err != nil {
		return auth, err
	}
...
func (e *CodexExecutor) PrepareRequest(req *http.Request, auth *cliproxyauth.Auth) error {
	...
	if helps.IsAgentIdentityAuth(auth) {
		authHeader, err := buildCodexAgentIdentityAuthorization(auth)
		if err != nil {
			return err
		}
...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		b = helps.RedactAgentIdentitySecrets(auth, b)
...
func (e *CodexExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		data = helps.RedactAgentIdentitySecrets(auth, data)
...
func applyCodexHeadersFromSources(r *http.Request, auth *cliproxyauth.Auth, token string, stream bool, cfg *config.Config, ginHeaders http.Header) error {
	...
	removeCodexUpstreamHopByHopHeaders(r.Header)
	return nil
}

func removeCodexUpstreamHopByHopHeaders(headers http.Header) {
	if headers == nil {
		return
	}
...
func buildCodexAgentIdentityAuthorization(auth *cliproxyauth.Auth) (string, error) {
	key, err := helps.AgentIdentityKeyFromAuth(auth)
	if err != nil {
		return "", err
	}
	return helps.BuildAgentAssertion(key, time.Now())
}

...
// TaskRecoverer lets an executor repair request-scoped credential state after
// an upstream response proves that a stored task identifier is invalid.
type TaskRecoverer interface {
	ShouldRecoverTask(auth *Auth, err error) bool
	RecoverTask(ctx context.Context, auth *Auth) (*Auth, error)
}
...
func (m *Manager) tryRefreshAfterUnauthorized(ctx context.Context, auth *Auth, execErr error, alreadyTried bool) (*Auth, bool) {
	...
	if recovered, okRecover := m.tryRecoverTaskAfterError(ctx, auth, execErr); okRecover {
...
func (m *Manager) tryRecoverTaskAfterError(ctx context.Context, auth *Auth, execErr error) (*Auth, bool) {
	...
	m.mu.RUnlock()
	recoverer, ok := exec.(TaskRecoverer)
	if !ok || recoverer == nil || !recoverer.ShouldRecoverTask(auth, execErr) {
		return auth, false
	}
	updated, errRecover := recoverer.RecoverTask(ctx, auth.Clone())
	if errRecover != nil || updated == nil {
		log.Debugf("task recovery before fallback failed for %s (%s): %v", auth.Provider, auth.ID, errRecover)
		return auth, false
	}
	saved, errUpdate := m.Update(ctx, updated)
	if errUpdate != nil {
		log.Debugf("persist task recovery before fallback failed for %s (%s): %v", auth.Provider, auth.ID, errUpdate)
		return updated, true
	}
	if saved != nil {
		return saved, true
